...
def graph_dot(x_tmp, y_tmp, color):
    ...
    for x in x_tmp:
        ...
        dot = canvas.create_oval(x_c - 1, (y_c - 1), x_c + 1, (y_c + 1), fill=color, outline=color, width=1)
        dot_list.append(dot)
        i += 1
    """рисуем линию H = ln(M)"""
    M = float(ent1.get())
    y1 = math.log(M)
    cx = -x_left * dx;
    x_t = cx + x_right * dx - 30;
    y_t = (y_top - y1) * dy - 15
    punkline = canvas.create_line(cx, (y_top - y1) * dy, cx + x_right * dx, (y_top - y1) * dy, fill=color, width=4,
                                  dash=2)
    signln = canvas.create_text(x_t - 10, y_t - 5, text="H=ln(%d)" % M, fill=color, font=("Helvetica 12 bold"))
    punkline_list.append(punkline)
    ...
    sign_dot_list.append([sign, signln])
    return dot_list
...
def H3(q, N, M, F12):
    ...
    for i in range(1, N + 1):
        if i <= M and i % 2 != 0:
            lambda0.append((q ** 2 * (N + F12) + 1) / (M * q ** 2 * N + N))
        elif i <= M and i % 2 == 0:
            lambda0.append((q ** 2 * (N - F12) + 1) / (M * q ** 2 * N + N))
        else:
            lambda0.append(1 / (M * q ** 2 * N + N))
...
def H3_redraw():
    M = float(ent1.get())
    if (abs(F12) <= N and M > 0 and M <= N and M == float(int(M))):
        global H3_list  # создание глобального списка значений Y
        global H3_dots  # объявление глобальным списка значений точек на холсте

        # удаление старого графика
        if (len(H3_dots_list) > 0):
            for dot in H3_dots:
                canvas.delete(dot)  # у холста есть метод delete
            H3_dots_list.pop(len(H3_dots_list) - 1)
        if (len(sign_dot_list) > 0):
            canvas.delete(sign_dot_list[len(sign_dot_list) - 1])
            sign_dot_list.pop(len(sign_dot_list) - 1)
            canvas.delete(punkline_list[len(sign_dot_list) - 1])
            punkline_list.pop(len(sign_dot_list) - 1)

        H3_list = H3_gr(q_list, N, M, F12)
        cr = random.randint(0, len(colors) - 1)
        H3_dots = graph_dot(q_list, H3_list, colors[cr])
        H3_dots_list.append(H3_dots)
...
def clean_draw():
    ...
    # удаление старого графика
    if (index >= 0):
        for dot in H3_dots_list[index]:
            canvas.delete(dot)  # у холста есть метод delete
        canvas.delete(sign_dot_list[index][0])
        canvas.delete(sign_dot_list[index][1])
        sign_dot_list.pop(index)
        canvas.delete(punkline_list[index])
        punkline_list.pop(index)

        H3_dots_list.pop(index)
...
def clean_draw_all():
    if (len(sign_dot_list) > 0):
        for H3_dots in H3_dots_list:
            # удаление старого графика
            for dot in H3_dots:
                canvas.delete(dot)  # у холста есть метод delete
            canvas.delete(sign_dot_list[len(sign_dot_list) - 1][0])
            canvas.delete(sign_dot_list[len(sign_dot_list) - 1][1])
            sign_dot_list.pop(len(sign_dot_list) - 1)
            canvas.delete(punkline_list[len(sign_dot_list) - 1])
            punkline_list.pop(len(sign_dot_list) - 1)
...
""" 1. Добавление меток """
lab1 = tk.Label(panel1, text="Количество некоррелированных \n  сигналов с линейно независимыми\n\
ортогональными\n векторами (0 < M < N, целое) :", font='Times 12')
lab1.place(x=10, y=90, width=250, height=90)

lab2 = tk.Label(panel1, text="   M = ", font='Times 14')
lab2.place(x=50, y=230, width=30, height=50)

lab3 = tk.Label(panel2, text="Зависимость значений энтропии от ОСШ суммы " + "\n\
M некоррелированных  сигналов c линейно независимыми ортогональными" +
                             "\n векторами  " + "X\u2081" + ",..., " + "X\u2098" + "  и собственного шума \n (длина выборки процесса N = 10)",
                font='Times 14')
...
H3_dots_list = []
global punkline_list;
punkline_list = []
global sign_dot_list;
sign_dot_list = []  # подписи к графикам

H3_dots = graph_dot(q_list, H3_list, colors[cr])  # один график
H3_dots_list.append(H3_dots)  # все графики
